			...
			for (User u : lu) {
				...
				if (true) {
					try {
						...
						rateCheck();
						numOfUnfollowed++;
						global.Global.gout.println("unFollowed a user：\t"
								+ numOfUnfollowed);
	...
	private void followOthers(HashMap<Long, String> usersToAt) {
		for (String userName : usersToAt.values()) {
			try {
				...
				rateCheck();
				global.Global.gout.println("\t\tFollowed a user：\t");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	private void repostStatus(String str, int mode) {
		try {
			...
			for (Status st : statuses) {
				...
				rateCheck();
				global.Global.gout.println("转发了一条微博\t" + statusId + "\t"
						+ words);
